...
var video = document.querySelector('#webcam'),
	...
	curtainSize,
	useSIMD = true;
document.querySelector('#source').addEventListener('mousemove', function (e) {
	var RGB = getRgb(e.offsetX, e.offsetY);
	  ...
	  document.querySelector('.color .red').text = RGB[0];
	  document.querySelector('.color .green').text = RGB[1];
	  document.querySelector('.color .blue').text = RGB[2];
});
...
var setUIInput = function (defaultMatrix, defaultDivisor, defaultOffset) {
	for (var row = 0; row <= 2 ; row++) {
		for (var col = 0; col <= 2; col++) {
			document.querySelector("#r" + row + "_c" + col).value = defaultMatrix[row][col];
		}
	}
	document.querySelector(".divisor-value").value = defaultDivisor;
	document.querySelector(".offset-value").value = defaultOffset;
};
...
var changeEffects = function () {
	...
	var data = effects[document.querySelector("#effects").value];
...
var bindUIInput = function () {
	...
	// 	$("#effects").val("custom");
	// });
	document.querySelector("#effects").addEventListener('change', function () {
		changeEffects();
	});
};
...
setInterval(function () {
	document.querySelector(".fps span").text = fps.toFixed(2);
}, 1000);
